class CopyManga extends ComicSource {
    ...
    url = "https://github.com/lingxidev/venconfigs/blob/main/copyManga.js"
    ...
    static defaultImageQuality = "800"

    static defaultApiUrl = 'api.2025copy.com'

    static searchApi = "/api/kb/web/searchb/comics"

    base_url = CopyManga.defaultApiUrl;
    ...
    explore = [
         {
            ...
            load: async (page) => {
                ...
                let category_url;
                category_url = `${this.apiUrl}/api/v3/comics?limit=30&offset=0&ordering=-datetime_updated&theme=maoxian&top=`
        ...
        loadInfo: async (id) => {
            let getChapters = async (id, groups) => {
                let fetchSingle = async (id, path) => {
                    ...
                    //     },
                    //     "const container = document.querySelector('.response-info .prettyprint');const allChildren = container.querySelectorAll('*');const result2 = Array.from(allChildren).filter(el => !el.closest('.nocode')).map(el => { if (el.tagName.toLowerCase() === 'a') {return ''} else {return el.innerText} });result2.join('');"
                    // );
                    if (res.status !== 200) {
                        throw `Invalid status code: ${res.status}`;
                    }
            ...
            // let homeRes = await InAppOpen.load(
            //     `${this.apiUrl}/api/v3/comic2/${id}?in_mainland=true&_update=true&platform=1`,
            //     {
            //         "User-Agent":
            //         "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
            //     },
            ...
            let homeRes = await Network.get(`${this.apiUrl}/api/v3/comic2/${id}?in_mainland=true&_update=true&platform=1`,this.headers);
            if (homeRes.status !== 200) {
                throw `Invalid status code: ${homeRes.status}`;
            }
            let data = JSON.parse(homeRes.body).results;
    ...
    async refreshAppApi() {
        ...
        if (res.status === 200) {
            ...
            this.base_url= data.results.api[0][0];
        }
    }
}