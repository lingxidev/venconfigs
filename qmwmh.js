class NewComicSource extends ComicSource {  // 首行必须为class...
    ...
    // 更新链接
    url = "https://github.com/lingxidev/venconfigs/blob/main/qmwmh.js"
    ...
    account = {
        ...
        login: async (account, pwd) => {
            let res = await Network.post("http://45.142.157.135:8080/api/user/userarr/login", {
        ...
        registerWebsite: "http://45.142.157.135:8080/user/register/"
    }
    parseComic(e) {
    ...
    explore = [
        {
            ...
            */
            load: async () => {
                let res = await Network.get("http://45.142.157.135:8080", {
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
                })
                if (res.status !== 200) {
                    throw "Invalid status code: " + res.status
    ...
    categoryComics = {
        load: async (category, param, options, page) => {
            ...
            if (param !== undefined && param !== null) {
                url = `http://45.142.157.135:8080/l/9/${category}/3/1.html"`
            } else {
                url = `http://45.142.157.135:8080/l/${options[1]}/${category}/${options[0]}/${page}.html`
            }
    ...
    search = {
        load: async (keyword, options, page) => {
            let res = await Network.get(`http://45.142.157.135:8080/api/front/index/search?key=${encodeURIComponent(keyword)}`, {
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            })
            if (res.status !== 200) {
                throw "Invalid status code: " + res.status
    ...
    favorites = {
        ...
        addOrDelFavorite: async (comicId, folderId, isAdding) => {
            ...
            if (isAdding) {
                ...
                let res = await Network.post("http://45.142.157.135:8080/api/user/bookcase/add", {
                ...
                let res = await Network.post("http://45.142.157.135:8080/api/user/bookcase/del", {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
                }, `articleid=${id}`)
                if (res.status !== 200) {
        ...
        loadComics: async (page, folder) => {
            let res = await Network.post("http://45.142.157.135:8080/api/user/bookcase/ajax", {
                "Content-Type": "application/x-www-form-urlencoded",
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            }, `page=${page}`)
            if (res.status !== 200) {
            ...
            let comics = json["data"].map(e => {
                return {
                    ...
                    subTitle: e["author"],
                    cover: e["cover"],
                    id: "http://45.142.157.135:8080" + e["info_url"]
                }
            })
    ...
    comic = {
        // 加载漫画信息
        loadInfo: async (id) => {
            let res = await Network.get(`http://45.142.157.135:8080${id}`,
                {
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
                }
            )
        ...
        loadEp: async (comicId, epId) => {
            ...
            let document = new HtmlDocument(res.body)
            return {
                images: document.querySelectorAll(".acgn-reader-chapter__item-box .acgn-reader-chapter__item img").map(e => e.attributes["src"])
            }
        },
        /// 警告: 这是历史遗留问题, 对于新的漫画源, 不应当使用此字段, 在选取漫画id时, 不应当出现特殊字符
        matchBriefIdRegex: "http://45.142.157.135:8080/(\\d+)/"
    }
}
